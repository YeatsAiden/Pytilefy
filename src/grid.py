...
class Grid(pg.sprite.Sprite):
    ...
    def update(self, camera) -> None:
        ...
        for y in range(int(camera.scroll.y)//TILE_SIZE, (int(camera.scroll.y) + DISPLAY_HEIGHT)//TILE_SIZE + 1):
            pg.draw.line(self.image, (50, 50, 50), (0, y * TILE_SIZE - camera.scroll.y), (DISPLAY_WIDTH, y * TILE_SIZE - camera.scroll.y), 2)

        for x in range(int(camera.scroll.x)//TILE_SIZE, (int(camera.scroll.x) + DISPLAY_WIDTH)//TILE_SIZE + 1):
